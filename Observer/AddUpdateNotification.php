 ...
 * of the blog module is published to the eTechFlow private Composer repo. Fires
 * when the Blog Posts admin page is opened.
 *
 * Re-surfacing rule: the notice is (re-)added whenever it is not currently
 * present in the inbox — so if the admin dismisses/removes it, opening the
 * page brings it back while an update is still pending. It is NOT duplicated
 * when an active entry for the same version already exists. Fully fail-safe.
 */
class AddUpdateNotification implements ObserverInterface
{
    private const PACKAGE     = 'etechflow/module-blog';
    private const LATEST_URL  = 'https://license-service.etechflow.com/composer/latest/etechflow/module-blog.json';
    private const CACHE_KEY   = 'etechflow_blog_latest_version';
    ...
    public function execute(Observer $observer)
    {
        try {
            ...
            $title = (string) __('eTechFlow Blog %1 is available', $latest['version']);
