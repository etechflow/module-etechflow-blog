...
class Activated extends Action
{
    ...
    public function execute(): ResultInterface
    {
        ...
        if (!$sessionId) {
            ...
            return $this->resultFactory->create(ResultFactory::TYPE_REDIRECT)->setPath('etechflow_blog/license/gate');
        }
        ...
        $block = $page->getLayout()->getBlock('etechflow.blog.license.activated');
        if ($block) {
            $block->setData('license_key', $licenseKey)
                  ->setData('plan', $planName)
                  ->setData('error', $error)
                  ->setData('settings_url', $this->getUrl('adminhtml/system_config/edit/section/etechflow_blog'))
                  ->setData('management_url', $this->getUrl('etechflow_blog/license/gate'));
        }
